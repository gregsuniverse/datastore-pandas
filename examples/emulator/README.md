...
```powershell
python examples\emulator\generate_mock_data.py --rows 50000 --out examples\emulator\data\workouts.csv
python examples\emulator\load_mock_data.py --input examples\emulator\data\workouts.csv --workers 8
python examples\emulator\query_examples.py --user-id user-00042
python examples\emulator\patch_sparse_rows.py --user-id user-00042
python examples\emulator\transaction_example.py
...
the official trip-history bucket, reads it into pandas, and maps it into this
Datastore hierarchy:
...
It writes both `Station` and `Ride` entities through `datastore-pandas`, then
checks:
